...
class Sequence
{
     ...
     * @param bool $hideDate 是否隐藏日期，true-通过十六进制转换
     * @return string
     */
    public static function getNextGlobalId($tablename, $uid, $isUseUid = FALSE, $hideDate = FALSE){
        $seq = '';
        ...
        if($isUseUid) {
            ...
            $id = sprintf('%s%06s%03s',self::getTimestampSeq($hideDate), $seq, $uid);
        } else {
            //$id = self::getTimestampSeq()*10000000 + $seq*1000 + self::getStrIndex($uid);
            $id = sprintf('%s%06s%03s',self::getTimestampSeq($hideDate), $seq, self::getStrIndex($uid));
        }
     ...
     * @return string
     */
    public static function getNextGlobalSeq($tablename, $seq, $hideDate = FALSE)
    {
        if(mb_strlen($seq)<20) {
        ...
        return self::getNextGlobalId($tablename, $index, true, $hideDate);
    }
     ...
     */
    protected static function getTimestampSeq($hideDate){
        $time = time();
        $data = date('Ymd', $time);
        $second = $time - strtotime($data);
        $short_date = date('ymd', $time);
        $short_date = empty($hideDate) ? $short_date : sprintf("%06s", dechex($short_date));
        $id = $short_date . $second;
        return $id;
    }
