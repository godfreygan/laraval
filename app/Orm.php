...
abstract class Orm extends \Illuminate\Database\Eloquent\Model
{
    ...
    public static function spList($aCondition = [], $iPage = 1, $iPerPage = 10, $order = [], $iLastID = 0)
    {
        ...
        if ($aResult['total'] > 0) {
            //查询指定字段
            $aField = isset($aCondition['aField']) && is_array($aCondition['aField']) ? $aCondition['aField'] : ['*'];
            $aField = empty($aField) ? ['*'] : $aField;
            //获取排序条件
            $order = !empty($order) && is_array($order) ? $order : [$primaryKey => 'desc'];
            foreach ($order as $k => $v) {
                /*if (in_array($k, $columns) && in_array(strtolower($v), ['asc', 'desc'])) {
            ...
            if ($primaryKey && !empty($iLastID)) {
                ...
                $res    = $oModel->limit($iPerPage)->get($aField)->toArray();
            } else { //常规分页
                $offset = ($iPage - 1) * $iPerPage;
                $res    = $oModel->skip($offset)->take($iPerPage)->get($aField)->toArray();
            }
            $aResult['list'] = $res;
        }
        return $aResult;
    ...
    private static function handleOr($oModel, $val)
    {
     ...
     * @return array | mix
     */
    public static function spGetOne($aCondition = [])
    {
        $oModel = new static;
        return $oModel->getCondition($oModel, $aCondition)->first();
    }
